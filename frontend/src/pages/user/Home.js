...
import React, { useState, useEffect } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { FiArrowRight, FiTruck, FiShield, FiRefreshCw, FiHeadphones, FiStar, FiPackage } from 'react-icons/fi';
import { HiSparkles, HiArrowRight } from 'react-icons/hi2';
import { productAPI, categoryAPI } from '../../services/api';
import { bundleAPI } from '../../services/api'; // 👈 bundle API
import ProductCard from '../../components/common/ProductCard';
import LoadingSpinner from '../../components/common/LoadingSpinner';
...
const Home = () => {
  ...
  const [newArrivals, setNewArrivals]           = useState([]);
  const [categories, setCategories]             = useState([]);
  const [bundles, setBundles]                   = useState([]);       // 👈 bundles state
  const [loading, setLoading]                   = useState(true);
  const [bundlesLoading, setBundlesLoading]     = useState(true);    // 👈 separate loading
  const navigate = useNavigate();
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const subCats = all.filter(c => c.level === 1);
        const standaloneParents = all.filter(c => c.level === 0 && !all.some(ch => ch.level === 1 && (ch.parent?._id || ch.parent)?.toString() === c._id.toString()));
        setCategories([...subCats, ...standaloneParents].slice(0, 8));
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  return (
    <div className="pt-16">
      ...
      {/* Promo Video Section */}
      <section className="relative w-full h-[60vh] overflow-hidden">
        <video src={homeVideo} autoPlay loop muted playsInline className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
          <div className="text-center text-white px-4">
            <h2 className="text-3xl md:text-5xl font-bold mb-4">Shop Smart. Shop Fast.</h2>
            <p className="mb-6 text-lg text-white/80">Discover trending products at best prices</p>
            <Link to="/products" className="bg-white text-black px-6 py-3 rounded-xl font-semibold hover:bg-gray-200 transition">
              Explore Now
            </Link>
          </div>
        </div>
      ...
      <section className="py-10 bg-white">
        <div className="page-container">
          <Swiper modules={[Autoplay]} autoplay={{ delay: 2500 }} loop spaceBetween={20} slidesPerView={1}>
            {[
              "https://images.unsplash.com/photo-1607083206968-13611e3d76db?w=1200",
              "https://images.unsplash.com/photo-1521334884684-d80222895322?w=1200",
              "https://images.unsplash.com/photo-1512436991641-6745cdb1723f?w=1200"
              ...
              <SwiperSlide key={i}>
                <div className="rounded-2xl overflow-hidden relative h-[250px] md:h-[350px]">
                  ...
                  <div className="absolute inset-0 bg-black/30 flex items-center">
                    <div className="text-white p-6">
                      <h3 className="text-2xl font-bold mb-2">Mega Sale {i + 1}</h3>
                      <p className="text-sm mb-3">Up to 50% OFF on trending items</p>
                      <Link to="/products" className="bg-white text-black px-4 py-2 rounded-lg text-sm font-semibold">Shop Now</Link>
                    </div>
                  </div>
                </div>
              </SwiperSlide>
            ...
            {loading ? (
              ...
              categories.map(cat => (
                <Link key={cat._id} to={`/products?category=${cat.slug}`}
                  ...
                  <span className="text-xs font-semibold text-gray-700 text-center group-hover:text-primary-600 transition-colors">{cat.name}</span>
                </Link>
              ))
            ) : (
              <p className="col-span-full text-center text-gray-800">No categories found 🙂</p>
            )}
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            {loading
              ...
              : featuredProducts.length > 0
                ...
                : <p className="col-span-full text-center text-gray-900">No featured products found🙂</p>
            }
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            {loading
              ...
              : newArrivals.length > 0
                ...
                : <p className="col-span-full text-center text-gray-900">No new arrivals found🙂</p>
            }
          </div>
        </div>
      </section>
